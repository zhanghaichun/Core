...
*/
    ...
    $('#' + this.submitBtn).bind('click', function() {
		...
		var myDate=new Date();
		var arr = $('#' + __method.mOpts.startDateId).val().split('-')
		myDate.setFullYear(arr[0],arr[1],arr[2]);
		$('#' + __method.inputId).attr('start',myDate.getTime())
		arr = $('#' + __method.mOpts.endDateId).val().split('-')
		myDate.setFullYear(arr[0],arr[1],arr[2]);
		$('#' + __method.inputId).attr('end',myDate.getTime())
        return false;
    });
    // 日期选择关闭按钮的 click 事件
